...
def get_data_excel_serial(serial_num = None):
    rows_global_2 = ()
    if serial_num:
        serial_num = '%' + serial_num + '%'
    try:
        ...
        the_query = """
            ...
            """
        cursor.execute(the_query, (serial_num,serial_num))
        rows = cursor.fetchall()
        rows_global_2 = rows
    except psycopg2.Error:
        return None
...
def get_data_excel_mixed(start_date = None, end_date = None, serial_num = None ):
    rows_global_2 = ()
    if serial_num:
        serial_num = '%' + serial_num + '%'
    try:
        ...
        the_query = """
                ...
                ORDER BY
                    serial_num, test_name, test_date DESC, test_time DESC;
            """
        cursor.execute(the_query, (start_date,start_date,end_date,serial_num,serial_num))
        rows = cursor.fetchall()
        rows_global_2 = rows
    except psycopg2.Error:
        return None
...
def get_data_excel_date(start_date = None, end_date = None):
    rows_global_2 = ()
    try:
        conn = psycopg2.connect(
            dbname="devices",
        ...
        secondd_query = """
                        ...
                        ORDER BY
                            MAX(test_date || ' ' || test_time) DESC"""
        cursor.execute(secondd_query, (start_date, end_date))
        second_rowss = cursor.fetchall()
        serial_nums_tuplee = tuple(row[0] for row in second_rowss)
        print(serial_nums_tuplee)
        queryy = """
                    WITH RankedTestResults AS (
                        SELECT
                            serial_num,
                    ...
                    ORDER BY
                        MAX(test_date || ' ' || test_time) DESC;
                    """
        cursor.execute(queryy, (serial_nums_tuplee,))
        rows = cursor.fetchall()
        print(rows)
        rows_global_2 = rows
    except psycopg2.Error:
